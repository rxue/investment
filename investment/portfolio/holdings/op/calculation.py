...
from investment.portfolio.lots_matching import fifo_lots_matching, to_lots_by_company_symbol, UnrealizedLots
...
def extract_holdings_from_op_transaction_csvs(csv_directory: str, optional_fields:list[str]) -> tuple[list[Holding], list[str]]:
    ...
    lots_matching_result_by_company_symbol = {
        company_symbol: fifo_lots_matching(input_lots)[1]
        for company_symbol, input_lots in to_lots_by_company_symbol(tradings_df).items()
    }
    def to_holding(company_symbol:str, unrealized_lots:UnrealizedLots) -> Holding | None:
        ...
        if company is not None:
            ...
            return Holding(
                company=company,
                amount=unrealized_lots.holding(),
