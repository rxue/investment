...
if command == EXTRACT_HOLDINGS:
    if len(sys.argv) != 3:
        print(f"Usage: python -m investment.portfolio.holdings extract_nordea_excel <excel_file>")
        sys.exit(1)
...
if command == EXTRACT_RETURN_BREAKDOWN:
    if len(sys.argv) != 3:
        print(f"Usage: python -m investment.portfolio.holdings {EXTRACT_RETURN_BREAKDOWN} <pdf_file>")
        sys.exit(1)
    ...
    if len(sys.argv) != 3:
        print(f"Usage: python -m investment.portfolio.holdings {TOTAL_RETURN} <pdf_dir>")
        sys.exit(1)
