...
def get_quotes(*symbols: str, multithreads: bool = False, date: date | None = None) -> list[Quote]:
    if multithreads:
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = {executor.submit(get_quote, symbol, date): symbol for symbol in symbols}
            return [quote for future in as_completed(futures) if (quote := future.result()) is not None]
    else:
        return [quote for symbol in symbols if (quote := get_quote(symbol, date)) is not None]
